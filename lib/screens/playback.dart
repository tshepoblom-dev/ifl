...
class PlaybackPage extends StatefulWidget {
  ...
  final String name;
  final Device device;
  PlaybackPage(this.id, this.from, this.to, this.name, this.device);
  @override
...
class _PlaybackPageState extends State<PlaybackPage> {
  ...
  late StreamController<Position> _postsController;
  Timer? timerPlayBack;
  ...
  void playRoute() async {
    ...
    final Uint8List? icon = await Util().getBytesFromAsset(iconPath, 80);
    interval(new Duration(milliseconds: playbackTime), (timer) {
      if (routeList.length != _sliderValue) {
        _sliderValue++;
      }
      timerPlayBack = timer;
      _markers.removeWhere((m) => m.markerId.value == widget.id.toString());
  ...
  getReport() {
    getStops();
    Traccar.getPositions(widget.id.toString(), widget.from, widget.to)
        .then((value) => {
              if (value!.length != 0)
                {
